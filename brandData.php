...
<li class="active treeview">
 ...
 <ul class="treeview-menu">
    ...
    <li class=" active treeview">
          ...
          <ul class="treeview-menu">
            <li class="active"><a href="brandData.php"><i class="fa fa-database"></i> Daftar Brand</a></li>
        ...
        <div class="col-md-12">
          <div class="box">
            <div class="box-header with-border">
              <div class="col-md-6">
                <span class="box-title">Daftar Seluruh Brand </span> 
                <a class='btn btn-xs  btn-success' href="brandTambah.php"><i class ='fa fa-plus'></i> Tambah Brand </a>
              </div>
              <div class="box-tools">
...
<?php  
    ...
    $sql = "SELECT * FROM as_brands";
      ...
      $totalRow = mysqli_num_rows($brandData);
      ...
      $sqlHal = mysqli_query($conn," SELECT * FROM as_brands LIMIT $start, $perHalaman");
      // variable nomor urut
      $nomor = $start+1;
      ...
      $tbl.= "<th>No</th>";
      $tbl.= "<th>id brand</th>";
      $tbl.= "<th>nama brand</th>";
      $tbl.= "<th>status brand</th>";
      $tbl.= "<th>tanggal dibuat</th>";
      $tbl.= "<th>nama pembuat</th>";
      $tbl.= "<th>tanggal di edit</th>";
      $tbl.= "<th>nama pengedit</th>";
      $tbl.= "<th>aksi</th>";
      $tbl.= "</tr>";
      // variable nomor urut
      ...
      if (mysqli_num_rows($sqlHal) > 0) {
        ...
        while ( $value = mysqli_fetch_assoc($sqlHal)) {
          ...
          $tbl.= "<tr>";
          $tbl.= "<td>".$nomor."</td>";
          $tbl.= "<td>".$value['brandID']."</td>";
          $tbl.= "<td>".$value['brandName']."</td>";
          $status = $value['brandStatus'];
          $y="aktif";
          if ($status != "Y") {
            $y="tidak aktif";
          }
          ...
          $sql_createID= "SELECT * FROM as_users 
              WHERE userID='$createdID'";
              $query_creaedID = mysqli_query($conn,$sql_createID);
              $createdID1= mysqli_num_rows($query_creaedID);
              if ($createdID1 > 0) {
                 foreach ($query_creaedID as $key => $isi) { 
                       $tbl.= "<td>".$isi['userFullName']."</td>";    
                  }
              }
          ...
          $sql_Modified= "SELECT * FROM as_users 
              ...
              if ($modifiedID > 0) {
                 foreach ($query_modifiedID as $key =>$modif) { 
                       $tbl.= "<td>".$modif['userFullName']."</td>";   
                  }
              }
              else{
           ...
           $tbl.= "<td>
           <a class='btn btn-xs btn-primary' href='brandDetail.php?brandID=".$value['brandID']."'><i class='fa fa-eye'></i></a>
          <a class='btn btn-xs btn-warning' href=brandUpdate.php?brandID=".$value['brandID']."><i class='fa fa-edit'></i></a>
          <a class='btn btn-xs btn-danger' href='javascript:hapus(".$value['brandID'].")'><i class='fa fa-trash'></i></a></td>";
          $tbl.= "</tr>";
